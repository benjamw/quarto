...
// TODO: convert the following to something like on pharaoh
			...
			if (is_array($list)) {
				foreach ($list as $game) {
					...
					if ($game['in_game'] && ! in_array($game['state'], ['Finished', 'Draw'])) {
						++$menu_data['my_games'];
					}
	...
	<aside id="nav">';
		...
		<nav id="mygames" class="box">
			...
			foreach ($game_data as $game) {
				$class = ($game['my_turn']) ? 'playing' : ((in_array($game['state'], ['Finished', 'Draw'])) ? 'finished' : 'waiting');
				$html .= '
